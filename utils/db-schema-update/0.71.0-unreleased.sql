...
CREATE TABLE IF NOT EXISTS `chat_invite_link` (
  `id` BIGINT UNSIGNED COMMENT 'Unique identifier for this entry'
      PRIMARY KEY,
  -- `invite_link` VARCHAR(2083) CHARACTER SET 'ascii' COLLATE 'ascii_general_ci' NOT NULL
  -- IE7 has a 2083 character limit for HTTP GET operations: http://support.microsoft.com/kb/208427
  `invite_link` VARCHAR(255) CHARACTER SET 'ascii' COLLATE 'ascii_general_ci' NOT NULL COMMENT 'The invite link. If the link was created by another chat administrator, then the second part of the link will be replaced with “…”',
  -- @noplanman We should be safe with much less here, I really don't think an invite link will have that many characters.
